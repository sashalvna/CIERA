...
for elems in stable_MT_index:
    ...
    delta_m2 = m2-m2_all
    maxdelta_m2 = max(delta_m2)
    f1.append([elems, m1_all, m2_all, p_all, mdot_all, wcrit, maxdelta_m2])
f1 = np.array(f1)
f1 = np.transpose(f1)
...
for elems in unstable_MT_index:
    ...
    delta_m2 = m2-m2_all
    maxdelta_m2 = max(delta_m2)
    f2.append([elems, m1_all, m2_all, p_all, mdot_all, wcrit, maxdelta_m2])
f2 = np.array(f2)
f2 = np.transpose(f2)
...
for elems in no_MT_index:
    ...
    delta_m2 = m2-m2_all
    maxdelta_m2 = max(delta_m2)
    f3.append([elems, m1_all, m2_all, p_all, mdot_all, wcrit, maxdelta_m2])
f3 = np.array(f3)
f3 = np.transpose(f3)
...
for elems in notconverged_MT_index:
    ...
    delta_m2 = m2-m2_all
    maxdelta_m2 = max(delta_m2)
    f4.append([elems, m1_all, m2_all, p_all, mdot_all, wcrit, maxdelta_m2])
...
for m_CO in m_COs:
    ...
    plt.colorbar(orientation='horizontal', label=r'$\mathrm{max}$ $ \Delta M_2$')
    plt.clim(minm2, maxm2)
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    #plt.legend()

    plt.savefig('zsolarwrlof_m_CO_%.3s_m2_deltam2.png'%m_CO, bbox_inches='tight')
    plt.show()
    notconverged += len(nc)
