...
for elems in stable_MT_index:
    ...
    porb_final = grid[elems].final_values['period_days']
    porb_final = np.log10(porb_final)
    f1.append([elems, m1_all, m2_all, p_all, mdot_all, porb_final, m2_final])
f1 = np.array(f1)
f1 = np.transpose(f1)
...
for elems in unstable_MT_index:
    ...
    porb_final = grid[elems].final_values['period_days']
    porb_final = np.log10(porb_final)
    f2.append([elems, m1_all, m2_all, p_all, mdot_all, porb_final, m2_final])
f2 = np.array(f2)
f2 = np.transpose(f2)
...
for elems in no_MT_index:
    ...
    porb_final = grid[elems].final_values['period_days']
    porb_final = np.log10(porb_final)
    f3.append([elems, m1_all, m2_all, p_all, mdot_all, porb_final, m2_final])
f3 = np.array(f3)
f3 = np.transpose(f3)
...
for elems in notconverged_MT_index:
    ...
    porb_final = grid[elems].final_values['period_days']
    porb_final = np.log10(porb_final)
    f4.append([elems, m1_all, m2_all, p_all, mdot_all, porb_final, m2_final])
...
for m_CO in m_COs:
    ...
    try:
        minm2 = 3 #min(min(f3[5][ind3]), min(tpagb[2]))
        maxm2 = 6 #max(max(f3[5][ind3]), max(tpagb[2]))
    except: continue
    ...
    #try: plt.scatter(np.log10(tpagb[0]), np.log10(tpagb[1]), c=tpagb[2], vmin=minm2, vmax=maxm2, marker='*', s=20, cmap='viridis', label='Reached TPAGB')
    #except: pass
    ...
    plt.colorbar(orientation='horizontal', label=r'$\log_{10}(P_\mathrm{orb})$')
    plt.clim(minm2, maxm2)
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    #plt.legend()

    plt.savefig('zsolarwrlof_m_CO_%.3s_m2_final_p_orb.png'%m_CO, bbox_inches='tight')
    plt.show()
    notconverged += len(nc)
