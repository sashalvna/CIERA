...
for elems in stable_MT_index:
    ...
    f1.append([elems, m1_all, m2_all, p_all, mdot_all, wcrit, max_mdot])
f1 = np.array(f1)
f1 = np.transpose(f1)
...
for elems in no_MT_index:
    ...
    wcrit = grid[elems].final_values['S2_surf_avg_omega_div_omega_crit']
    m2_final = grid[elems].final_values['star_2_mass']
    f3.append([elems, m1_all, m2_all, p_all, mdot_all, wcrit, max_mdot])
f3 = np.array(f3)
f3 = np.transpose(f3)
...
for m_CO in m_COs:
    ...
    plt.savefig('zsolarwrlof_m_CO_%.3s_m2_mdot.png'%m_CO, bbox_inches='tight')
    plt.show()
    notconverged += len(nc)
