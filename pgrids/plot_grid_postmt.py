...
for elems in stable_MT_index:
    ...
    mdot2 = grid[elems].binary_history['lg_mtransfer_rate']
    max_mdot = max(grid[elems].binary_history['lg_mtransfer_rate'])
    index1 = np.where(mdot2 == max_mdot)[0][-1]
    mdot2_1 = mdot2[index1:]
    index2 = np.where(mdot2_1 <= -20)[0][0]
    index = index1+index2
    m2postmt = grid[elems].binary_history['star_2_mass'][index2]
    ...
    f1.append([elems, m1_all, m2_all, p_all, mdot_all, wcrit, m2postmt])
f1 = np.array(f1)
f1 = np.transpose(f1)

for elems in unstable_MT_index:
    mdot_all = grid[elems].binary_history['lg_mtransfer_rate'][-1]
    max_mdot = max(grid[elems].binary_history['lg_mstar_dot_2'])
    m1_all = grid[elems].binary_history['star_1_mass'][0]
    m2_all = grid[elems].binary_history['star_2_mass'][0]
    p_all = grid[elems].binary_history['period_days'][0]
    wcrit = grid[elems].final_values['S2_surf_avg_omega_div_omega_crit']
    m2_final = grid[elems].final_values['star_2_mass']
    f2.append([elems, m1_all, m2_all, p_all, mdot_all, wcrit, max_mdot])
f2 = np.array(f2)
f2 = np.transpose(f2)

for elems in no_MT_index:
    mdot_all = grid[elems].binary_history['lg_mtransfer_rate'][-1]
    mdot2 = grid[elems].binary_history['lg_mstar_dot_1']
    max_mdot = max(grid[elems].binary_history['lg_mstar_dot_1'])
    index1 = np.where(mdot2 == max_mdot)[0][-1]
    mdot2_1 = mdot2[index1:]
    try:
        index2 = np.where(mdot2_1 <= -12)[0][0]
        index = index2
        m2postmt = grid[elems].binary_history['star_2_mass'][index2]
    except: m2postmt = 0
    m1_all = grid[elems].binary_history['star_1_mass'][0]
    ...
    wcrit = grid[elems].final_values['S2_surf_avg_omega_div_omega_crit']
    m2_final = grid[elems].final_values['star_2_mass']
    f3.append([elems, m1_all, m2_all, p_all, mdot_all, wcrit, m2postmt])
f3 = np.array(f3)
f3 = np.transpose(f3)
...
for elems in notconverged_MT_index:
    ...
    wcrit = grid[elems].final_values['S2_surf_avg_omega_div_omega_crit']
    m2_final = grid[elems].final_values['star_2_mass']
    f4.append([elems, m1_all, m2_all, p_all, mdot_all, wcrit, max_mdot])
...
for m_CO in m_COs:
    ...
    for i in range(len(f3[6][ind3])):
        if f3[6][ind3][i] == 0:
            nomt.append([f3[1][ind3][i], f3[3][ind3][i]])
        else:
            mt.append([f3[1][ind3][i], f3[3][ind3][i], f3[6][ind3][i]])
    ...
    plt.scatter(np.log10(mt[0]), np.log10(mt[1]), c= mt[2], vmin = minm2, vmax = maxm2, marker='s', s=30, cmap='viridis')
    ...
    try: plt.scatter(np.log10(nomt[0]), np.log10(nomt[1]), c='gray', marker='D', s=15, label='No MT')
    except: pass
    ...
    plt.colorbar(orientation='horizontal', label=r'$\mathrm{max}$ $\log_{10}(\dot{M_2})$')
    plt.clim(minm2, maxm2)
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    #plt.legend()

    plt.savefig('zsolarwrlof_m_CO_%.3s_m2_after_mt.png'%m_CO, bbox_inches='tight')
    plt.show()
    notconverged += len(nc)
